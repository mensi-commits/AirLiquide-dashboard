...
export default function LogisticsDashboard() {
    ...
    // Auto-generate Lot ID based on new logic: GAZ-YY-MM-DD-SEQ or O2-YY-MM-DD-3C
    const generateLotId = (gas, isCiterne, citerneType) => {
        ...
        if (isCiterne && gas === "O2") {
            return `${gas}-${yy}-${mm}-${dd}-${citerneType}`;
        }
    ...
    const fetchBatches = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Logistics only cares about Raw Materials and Citernes
    ...
    const handleAddBatch = async (e) => {
        ...
        const payload = {
            ...
            party: newBatch.isCiterne ? "citerne" : "logistics",
            status: "received",
        ...
        try {
            ...
            if (res.ok) {
                ...
                setNewBatch({ lotId: "", gasId: "O2", quantity: "", supplier: "", isCiterne: false, citerneType: "3C" });
                fetchBatches();
            }
        } catch (err) {
            console.error(err);
    ...
    const sendToCiterne = async (lotId) => {
        ...
        const dateStr = parts.length >= 4 ? `${parts[1]}-${parts[2]}-${parts[3]}` : "26-08-04";
        const citerneCode = `O2-${dateStr}-3C`;
    ...
    const getStatusBadge = (batch) => {
        if (batch.status === "rejected") return { text: "Rejected", class: "bg-red-50 text-red-700 ring-red-200" };
        if (batch.type === "CITERNE") {
            if (batch.party === "citerne_lab") return { text: "Citerne In Lab", class: "bg-amber-50 text-amber-700 ring-amber-200" };
            if (batch.status === "approved") return { text: "Citerne Approved", class: "bg-emerald-50 text-emerald-700 ring-emerald-200" };
            return { text: "Citerne Pending", class: "bg-blue-50 text-blue-700 ring-blue-200" };
        }
        if (batch.party === "rm_lab") return { text: "In RM Quarantine", class: "bg-amber-50 text-amber-700 ring-amber-200" };
        if (batch.party === "production" || batch.status === "approved") return { text: "RM Approved", class: "bg-emerald-50 text-emerald-700 ring-emerald-200" };
        return { text: "Received", class: "bg-slate-100 text-slate-700 ring-slate-200" };
    };
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-40">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-900">Raw Materials Intake</h1>
                        <p className="text-xs text-slate-500">Manage and track all received raw materials & O₂ Citernes</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                        <div className="text-right">
                            <div className="text-sm font-semibold text-slate-900">Logistics Team</div>
                            <div className="text-xs text-slate-500">Intake & Quarantine</div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-600 mt-0.5">Total RM Received</div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-600 mt-0.5">In RM Quarantine</div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-600 mt-0.5">RM Approved</div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-600 mt-0.5">O₂ Citerne Process</div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 p-4">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                            <div className="flex-1 min-w-[260px]">
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Search by lot ID, material, supplier..."
                                        value={searchQuery}
                            ...
                            >
                                ...
                                <option>In RM Quarantine</option>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        {batch.party === "logistics" && (batch.type === "RM" || !batch.type) && (
                                                            ...
                                                            >
                                                                Send to RM Lab <ArrowRight className="h-3 w-3" />
                                                            </button>
                                                        )}
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200">
                            <div className="text-sm text-slate-500">
                                Showing <span className="font-medium text-slate-700">{paginatedBatches.length}</span> of{" "}
                                <span className="font-medium text-slate-700">{filteredBatches.length}</span> entries
                            </div>
                ...
                {showDetailsPanel && selectedBatch && (
                    <aside className="w-96 bg-white border-l border-slate-200 p-6 overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Lot ID</div><div className="text-sm font-bold text-slate-900 font-mono">{selectedBatch.lotId}</div></div>
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Gas Type</div><div className="text-sm font-medium text-slate-900">{selectedBatch.gasId}</div></div>
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Type</div><div className="text-sm font-medium text-slate-900">{selectedBatch.type || "RM"}</div></div>
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Supplier</div><div className="text-sm font-medium text-slate-900">{selectedBatch.supplier}</div></div>
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Received Date</div><div className="text-sm font-medium text-slate-900">{new Date(selectedBatch.date).toLocaleString()}</div></div>
                            <div><div className="text-xs font-medium text-slate-500 mb-1">Quantity</div><div className="text-sm font-medium text-slate-900">{selectedBatch.quantity} kg</div></div>
                        </div>
                    </aside>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleAddBatch} className="space-y-4">
                            <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Material Type (Gas)</label>
                                <select
                            ...
                            {newBatch.gasId === "O2" && (
                                <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                    ...
                                    <label htmlFor="isCiterne" className="text-sm font-medium text-amber-800">
                                        Route directly to O₂ Citerne (3C, 4C, 7C)
                                    </label>
                                </div>
                            )}
                            ...
                            {newBatch.isCiterne && newBatch.gasId === "O2" && (
                                <div>
                                    <label className="mb-1.5 block text-sm font-semibold text-slate-700">Citerne Type</label>
                                    <select
                                        className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm focus:border-blue-600 focus:outline-none"
                            ...
                            <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Batch / Lot No.</label>
                                <input
                                    required
                                    type="text"
                                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm focus:border-blue-600 focus:outline-none font-mono"
                                    value={newBatch.lotId}
                                    onChange={(e) => setNewBatch({ ...newBatch, lotId: e.target.value })}
                                    placeholder="e.g., O2-26-08-04-01"
                                />
                                <p className="text-xs text-slate-500 mt-1">Format: GAZ-YY-MM-DD-SEQ (or GAZ-YY-MM-DD-3C for Citernes)</p>
                            </div>

                            <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Quantity (kg)</label>
                                <input
                                    required
                                    type="text"
                                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm focus:border-blue-600 focus:outline-none"
                            ...
                            <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Supplier</label>
                                <input
                                    required
                                    type="text"
                                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm focus:border-blue-600 focus:outline-none"
